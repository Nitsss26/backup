...
// //       <div ref={mapContainerRef} className="flex-grow w-full min-h-[300px] sm:min-h-[400px] lg:min-h-[500px]" id="map"></div>
// //       <div className="p-4 bg-muted/50 rounded-b-md">
// //         <p className="text-sm font-medium">Selected Address:</p>
// //         <p className="text-xs text-muted-foreground min-h-[2.5rem] line-clamp-2">{address || "Click on the map or use search to set a location."}</p>
// //         <Button onClick={handleConfirm} disabled={!position || !address} className="w-full mt-2">
...
const LocationPicker: React.FC<LocationPickerProps> = ({ onLocationSelect }) => {
  ...
  useEffect(() => {
    if (mapContainerRef.current && !mapRef.current) {
      ...
      getUserLocation().then((location) => {
        if (location) {
          const userLatLng = { lat: location.latitude, lng: location.longitude };
          map.flyTo(userLatLng, 13);
          updateMarkerAndPosition(userLatLng, 'Fetching your location...');
        }
      }).catch((error) => {
        console.warn("Could not get user location on init:", error);
      });
    }
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      {/* Map container with responsive and constrained height */}
      <div ref={mapContainerRef} className="w-full h-[300px] sm:h-[400px] md:h-[500px] lg:h-[600px] overflow-hidden" id="map"></div>
      <div className="p-4 bg-muted/50 rounded-b-md">
        <p className="text-sm font-medium">Selected Address:</p>
        <p className="text-xs text-muted-foreground min-h-[2.5rem] line-clamp-2">{address || "Click on the map or use search to set a location."}</p>
        <Button onClick={handleConfirm} disabled={!position || !address} className="w-full mt-2">
