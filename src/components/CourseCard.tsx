    ...
    <Card className="flex flex-col h-full overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 border hover:border-primary/50">
      <Link href={`/courses/${course.id}`} className="block group">
        <CardHeader className="p-0 relative">
          <Image
            ...
            width={600}
            height={400}
            className="object-cover w-full h-40 sm:h-40 md:h-48"
      ...
      <CardContent className="p-3 sm:p-4 flex-grow">
        <Link href={`/courses/${course.id}`} className="block">
          <Badge variant="outline" className="mb-1 sm:mb-2 text-xs px-1.5 py-0.5 sm:px-2">{course.category}</Badge>
        ...
        <div className="flex items-center gap-1 sm:gap-2 text-xs text-muted-foreground mb-1 sm:mb-2">
          {course.providerInfo?.logoUrl && (
            <Avatar className="h-4 w-4 sm:h-5 sm:w-5 border">
              ...
              <AvatarFallback className="text-[8px] sm:text-xs">{providerNameInitial}</AvatarFallback>
            </Avatar>
          )}
      ...
      <CardFooter className="p-2.5 sm:p-3 flex items-center justify-between border-t mt-auto">
        <div>
        ...
        <Button size="xs" asChild className="text-xs sm:text-sm px-2 py-1 h-auto sm:px-3 sm:py-1.5">
