...
export function SellerStoreBanner({ image, title, ctaText, ctaLink }: SellerStoreBannerProps) {
  return (
    <div className="relative rounded-lg overflow-hidden shadow-md bg-[--bg-medium] md:max-w-[1500px] max-w-full">
      <Image src={image} alt={title} width={1500} height={300} className="w-full h-[120px] md:h-[200px] object-cover opacity-70" />
      <div className="absolute inset-0 flex items-center justify-between p-6">
        <motion.h3
          className="text-2xl font-bold text-[--text-light]"
          initial={{ opacity: 0, x: -20 }}
