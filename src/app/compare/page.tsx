...
const comparisonFeatures: ComparisonFeature[] = [
  {
    ...
    label: 'Title',
    render: (course: Course) => (
      <Link href={`/courses/${course.id}`} className="font-semibold text-primary hover:underline line-clamp-3 block">
        {course.title}
      </Link>
    ),
    className: "text-sm leading-snug hover:text-primary/80",
  ...
  },
  {
    ...
    render: (course: Course) => (
      <div className="text-sm md:text-base font-bold text-foreground">
        ₹{course.price.toLocaleString('en-IN')}
        {course.originalPrice && (
          <span className="ml-1 text-xs text-muted-foreground line-through block md:inline">
            ₹{course.originalPrice.toLocaleString('en-IN')}
          </span>
        )}
      </div>
  ...
  { key: 'level', label: 'Level', className: "text-xs md:text-sm capitalize" },
  { key: 'duration', label: 'Duration', className: "text-xs md:text-sm" },
  {
    key: 'modulesCount',
    label: 'Modules',
    render: (course: Course) => course.curriculum?.length || 0,
  ...
  },
  {
    ...
    render: (course: Course) => (
      <div className="flex flex-col items-center text-center">
        {course.providerInfo?.logoUrl && (
          <Avatar className="h-6 w-6 md:h-8 md:w-8 mb-1 border">
            <AvatarImage src={course.providerInfo.logoUrl} alt={course.providerInfo.name || "Provider"} />
            <AvatarFallback className="text-xs">{(course.providerInfo.name || "P").substring(0, 1)}</AvatarFallback>
          </Avatar>
        )}
        <span className="text-xs line-clamp-2">{course.providerInfo?.name || course.instructor}</span>
        {course.providerInfo?.verified && <Badge variant="success" className="mt-1 text-xs px-1 py-0.5">Verified</Badge>}
      </div>
    ),
    className: "min-w-[100px] md:min-w-[120px]",
  },
  { key: 'category', label: 'Category', className: "text-xs md:text-sm" },
  { key: 'language', label: 'Language', className: "text-xs md:text-sm" },
  {
    key: 'certificateAvailable',
    label: 'Certificate',
    render: (course: Course) => course.certificateAvailable ? <CheckCircle className="h-4 w-4 md:h-5 md:w-5 text-green-500 mx-auto" /> : <XCircle className="h-4 w-4 md:h-5 md:w-5 text-red-500 mx-auto" />,
  },
  {
    key: 'studentsEnrolledCount',
    label: 'Students',
  ...
  },
  {
    key: 'moneyBackGuaranteeDays',
    label: 'Guarantee',
    render: (course: Course) => course.moneyBackGuaranteeDays ? `${course.moneyBackGuaranteeDays} days` : <XCircle className="h-4 w-4 md:h-5 md:w-5 text-red-500 mx-auto" />,
    className: "text-xs md:text-sm",
  },
  {
    key: 'freeTrialAvailable',
    label: 'Free Trial',
    render: (course: Course) => course.freeTrialAvailable ? <CheckCircle className="h-4 w-4 md:h-5 md:w-5 text-green-500 mx-auto" /> : <XCircle className="h-4 w-4 md:h-5 md:w-5 text-red-500 mx-auto" />,
  },
];
...
export default function CompareCoursesPage() {
    ...
    return (
      <div className="flex flex-col min-h-screen">
        ...
        <main className="flex-grow container py-8 text-center">
          <BarChartHorizontalBig className="h-12 w-12 text-primary animate-pulse mx-auto my-10" />
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header />
      <main className="flex-grow container py-4 md:py-8 px-2 md:px-6 bg-slate-50 dark:bg-slate-900">
        <Breadcrumbs items={breadcrumbItems} />
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 md:mb-8 gap-4">
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold font-headline flex items-center text-foreground">
            <BarChartHorizontalBig className="mr-2 md:mr-3 h-6 w-6 md:h-8 md:w-8 text-primary" /> 
            Course Comparison
          </h1>
          {selectedCourses.length > 0 && (
            <Button variant="outline" onClick={() => setSelectedCourses([])} className="text-sm">
              <X className="mr-2 h-4 w-4" /> Clear All
            </Button>
          )}
        </div>

        <Card className="mb-6 md:mb-8 shadow-lg border bg-card">
          <CardHeader>
            <CardTitle className="text-lg md:text-xl font-headline text-foreground">Select Courses to Compare</CardTitle>
            <CardDescription className="text-sm">Choose up to {MAX_COMPARE_ITEMS} courses from the dropdowns below.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
            {Array.from({ length: MAX_COMPARE_ITEMS }).map((_, index) => (
              <div key={`slot-${index}`}>
                {selectedCourses[index] ? (
                  <Card className="min-h-[180px] md:min-h-[200px] flex flex-col p-2 md:p-3 shadow-sm border border-primary/40 bg-card">
                    <Image 
                      src={selectedCourses[index].imageUrl || `https://placehold.co/160x90.png`} 
                      alt={selectedCourses[index].title} 
                      width={160} 
                      ...
                      data-ai-hint={`${selectedCourses[index].category} course compare slot`}
                    />
                    <p className="text-xs font-semibold line-clamp-2 text-center text-foreground flex-grow mb-2 h-6 md:h-8">
                      {selectedCourses[index].title}
                    </p>
                    <Button 
                      variant="destructive" 
                      size="sm" 
                      onClick={() => handleRemoveCourse(selectedCourses[index].id)} 
                      className="w-full text-destructive-foreground text-xs py-1 h-7 md:h-8"
                    >
                      <X className="h-3 w-3 md:h-3.5 md:w-3.5 mr-1" /> Remove
                    </Button>
                  </Card>
                ) : (
                  <Card className="border-2 border-dashed border-muted-foreground/30 hover:border-primary/70 transition-all duration-200 ease-in-out min-h-[180px] md:min-h-[200px] flex flex-col items-center justify-center p-3 md:p-4 text-center bg-card">
                    {isLoadingCourses ? (
                      <Loader2 className="h-8 w-8 md:h-10 md:w-10 text-muted-foreground/50 mb-2 animate-spin" />
                    ) : (
                      <PlusCircle className="h-8 w-8 md:h-10 md:w-10 text-muted-foreground/50 group-hover:text-primary transition-colors" />
                    )}
                    <p className="text-xs font-medium text-muted-foreground mb-2 group-hover:text-primary">
                      Add Course {index + 1}
                    </p>
                    <Select 
                      onValueChange={handleAddCourse} 
                      ...
                      <SelectContent>
                        <SelectGroup>
                          ...
                          {availableCourses.length === 0 && !isLoadingCourses && 
                            <SelectItem value="no_courses_available" disabled>No courses available for selection.</SelectItem>
                          }
                        </SelectGroup>
                      </SelectContent>
                    </Select>
        ...
        {selectedCourses.length > 0 ? (
          <div className="overflow-x-auto mt-6 md:mt-10 shadow-xl rounded-lg border bg-card">
            <table className={`min-w-[calc(150px+200px*${selectedCourses.length})] md:min-w-[calc(200px+250px*${selectedCourses.length})] w-full border-collapse bg-card`}>
              <thead>
                <tr className="bg-card">
                  <th className="sticky left-0 z-30 bg-card p-2 md:p-3 text-left text-xs md:text-sm font-semibold text-foreground border-b border-r border-muted-foreground/30">
                    <Image 
                      src={selectedCourses[0]?.imageUrl || `https://placehold.co/120x67.png`} 
                      alt="Feature Icon" 
                      width={100} 
                      height={56} 
                      className="rounded-md object-cover aspect-video mx-auto shadow-sm w-full max-w-[100px] md:max-w-[120px]"
                    />
                  </th>
                  {selectedCourses.map((course) => (
                    <th key={course.id} className="p-2 md:p-3 text-center border-b border-muted-foreground/30 bg-card">
                      <Link href={`/courses/${course.id}`} className="block">
                        <Image 
                          ...
                          width={100} 
                          height={56} 
                          className="rounded-md object-cover aspect-video mx-auto mb-1 md:mb-1.5 shadow-sm w-full max-w-[100px] md:max-w-[120px]"
                          data-ai-hint={`${course.category} compare table thumbnail`}
                        />
                      </Link>
                    </th>
              ...
              <tbody>
                {comparisonFeatures.map((feature, index) => (
                  <tr key={feature.key} className={`${index % 2 === 0 ? 'bg-card' : 'bg-card'} group`}>
                    <td className={`sticky left-0 z-20 ${index % 2 === 0 ? 'bg-card' : 'bg-card'} p-2 md:p-3 text-xs md:text-sm font-medium text-foreground border-b border-r border-muted-foreground/30 align-middle`}>
                      {feature.label}
                    </td>
                    {selectedCourses.map((course) => (
                      <td key={`${course.id}-${feature.key}`} className={`p-2 md:p-3 text-center text-xs md:text-sm align-middle border-b border-muted-foreground/30 ${feature.className || ''}`}>
                        {feature.render ? feature.render(course) : (getNestedValue(course, feature.key) ?? 'N/A')}
                      </td>
                    ))}
                  </tr>
                ...
                <tr className="bg-card">
                  <td className="sticky left-0 z-20 bg-card p-2 md:p-3 text-xs md:text-sm font-medium text-foreground border-r border-muted-foreground/30 align-middle">
                    Actions
                  </td>
                  {selectedCourses.map((course) => (
                    <td key={`action-${course.id}`} className="p-2 md:p-3 text-center align-middle border-muted-foreground/30">
                      <div className="flex flex-col gap-1 md:gap-2">
                        <Button size="sm" asChild className="text-xs h-7 md:h-8 w-full">
                        ...
                        <Button 
                          variant="outline" 
                          size="sm" 
                          className="text-xs h-7 md:h-8 w-full" 
          ...
          <Card className="text-center py-12 md:py-16 bg-card rounded-lg shadow-lg border">
            <CardHeader>
              <BarChartHorizontalBig className="h-12 w-12 md:h-16 md:w-16 text-primary mx-auto mb-4" />
              <CardTitle className="text-xl md:text-2xl font-semibold mb-2 text-foreground">Ready to Compare?</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-muted-foreground mb-6 max-w-md mx-auto text-sm md:text-base">
                Select at least one course using the slots above to start your side-by-side comparison and find the perfect fit for your learning goals.
              </p>
              <Image 
                src="https://img.freepik.com/free-vector/feedback-loop-concept-illustration_114360-21826.jpg?uid=R120730963&ga=GA1.1.1385959138.1748893744&semt=ais_hybrid&w=740" 
                alt="Illustration for selecting courses to compare" 
                width={300} 
                height={150} 
                className="mx-auto rounded-md shadow-sm max-w-[300px] md:max-w-[400px]"
                data-ai-hint="comparison chart analytics education"
              />
            </CardContent>
          </Card>
