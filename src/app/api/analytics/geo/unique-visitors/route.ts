...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get unique visitors by location (state and city)
    const geoData = await VisitEvent.aggregate([
      ...
      {
        $addFields: {
          // Extract location from user agent or IP (placeholder logic)
          state: {
            $cond: {
    ...
    console.error('Error fetching unique visitors geo data:', error);
