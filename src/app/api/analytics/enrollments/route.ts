...
export async function GET(request: Request) {
  try {
    ...
    const currentEnrollments = await Order.countDocuments({
      status: 'completed',
      orderDate: { $gte: start, $lte: end }
    });

    // Count completed orders in previous period
    const previousEnrollments = await Order.countDocuments({
      status: 'completed',
    ...
    const increment = currentEnrollments - previousEnrollments;
    const incrementPercentage = previousEnrollments > 0 
      ? ((increment / previousEnrollments) * 100).toFixed(1)
