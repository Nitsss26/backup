...
export default function SellCoursesPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow">
        {/* Hero Section */}
        <section className="bg-gradient-to-br from-primary/80 via-blue-600 to-blue-700 text-primary-foreground py-12 sm:py-16 md:py-20 lg:py-28">
          <div className="container grid grid-cols-1 md:grid-cols-2 items-center gap-6 sm:gap-8 md:gap-12">
            <div className="space-y-4 sm:space-y-6 text-center md:text-left">
              <UploadCloud className="h-12 w-12 sm:h-16 sm:w-16 mx-auto md:mx-0 mb-2 sm:mb-4 text-yellow-300" />
              <h1 className="text-white text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold font-headline leading-tight">
                Share Your Knowledge. Grow Your Reach.
              </h1>
              <p className="text-sm sm:text-base md:text-lg text-blue-100/90 mb-4 sm:mb-6 max-w-xs sm:max-w-md md:max-w-xl mx-auto md:mx-0">
                Become a seller on {APP_NAME} – the premier marketplace for online courses. We provide the tools, audience, and support you need to succeed.
              </p>
              <Button size="lg" className="bg-yellow-400 hover:bg-yellow-500 text-primary-foreground dark:text-slate-900 text-base sm:text-lg px-6 sm:px-8 py-3 sm:py-4" asChild>
                <Link className="text-white" href="/auth/register?role=provider">Start Selling Today</Link>
              </Button>
            </div>
            <div className="hidden md:flex justify-center">
        ...
        <section className="py-8 sm:py-12 md:py-16 lg:py-20 bg-background">
          <div className="container">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-6 sm:mb-8 md:mb-12 font-headline">
              Why Sell on <span className="text-primary">{APP_NAME}?</span>
            </h2>
              ...
              {benefits.map((benefit, index) => (
                <Card key={index} className="text-center shadow-lg hover:shadow-xl transition-shadow duration-300 border-t-4 border-primary bg-card flex flex-col">
                  <CardHeader className="items-center pt-4 sm:pt-6">
                    ...
                    <CardTitle className="text-lg sm:text-xl font-semibold font-headline">{benefit.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="pb-4 sm:pb-6 flex-grow">
                    <p className="text-xs sm:text-sm text-muted-foreground px-2 sm:px-3 leading-relaxed">{benefit.description}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        {/* How It Works Section */}
        <section className="py-8 sm:py-12 md:py-16 lg:py-20 bg-secondary/50">
          <div className="container">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-8 sm:mb-12 md:mb-16 font-headline">
              Simple Steps to Start Selling Your Courses
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 sm:gap-x-8 gap-y-6 sm:gap-y-8 items-center">
              <div className="hidden md:flex justify-center items-center p-4 sm:p-6 bg-gradient-to-tr from-blue-500 to-primary rounded-xl shadow-2xl">
                <Image
                  src="https://img.freepik.com/premium-vector/learning-online-digital-elearning-studying-process_199064-2401.jpg"
                  alt="Seller dashboard interface mockup showing course creation tools and analytics graphs"
                  width={450}
                  height={550}
                  className="rounded-lg shadow-xl w-full h-auto sm:h-[300px] md:h-[400px] lg:h-[550px]"
                  data-ai-hint="seller dashboard course creation analytics mockup interface"
                />
              </div>
              <div className="space-y-4 sm:space-y-6">
                {steps.map((step, index) => (
                  <div key={index} className="flex items-start gap-2 sm:gap-3">
                    <div className="flex-shrink-0 h-8 w-8 sm:h-10 sm:w-10 bg-primary text-primary-foreground rounded-full flex items-center justify-center font-bold text-base sm:text-lg shadow-md">
                      {index + 1}
                    </div>
                    <div>
                      <h3 className="text-base sm:text-lg font-semibold mb-1 font-headline">{step.title}</h3>
                      <p className="text-xs sm:text-sm text-muted-foreground leading-normal">{step.description}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section className="py-8 sm:py-12 md:py-16 lg:py-20 bg-background">
          <div className="container text-center max-w-xl sm:max-w-2xl md:max-w-3xl mx-auto">
            <div className="mx-auto bg-success/10 p-2 sm:p-3 md:p-4 rounded-full w-fit mb-4 sm:mb-6">
              <Verified className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 text-success text-green-500" />
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2 sm:mb-4 font-headline">
              A Platform Built on Trust & Quality
            </h2>
            <p className="text-muted-foreground mb-4 sm:mb-6 text-base sm:text-lg leading-relaxed">
              To maintain a high-quality and trustworthy marketplace for learners, all sellers undergo a verification process. This helps ensure the authenticity of course providers and the quality of educational content. The process typically involves submitting identification and proof of expertise or institutional legitimacy. Our team is here to guide you through it.
            </p>
            <Button variant="outline" size="lg" asChild className="text-sm sm:text-base border-primary text-primary hover:bg-primary/10">
              <Link href="/auth/register?role=provider">Learn About Seller Verification</Link>
            </Button>
          </div>
        </section>
        ...
        {/* Call to Action Section */}
        <section className="py-12 sm:py-16 md:py-20 lg:py-24 text-center bg-gradient-to-r from-primary/10 to-secondary/20">
          <div className="container">
            <Sparkles className="h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 text-primary mx-auto mb-3 sm:mb-4 md:mb-5" />
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6 font-headline">
              Ready to Inspire Learners Worldwide?
            </h2>
            <p className="text-muted-foreground mb-6 sm:mb-8 md:mb-10 max-w-xs sm:max-w-md md:max-w-xl mx-auto text-base sm:text-lg leading-relaxed">
              Join our growing community of esteemed educators, institutions, and individual experts. We provide the platform, tools, and dedicated support you need to turn your passion into a thriving online teaching business.
            </p>
            <Button size="lg" className="text-base sm:text-lg px-6 sm:px-8 py-3 sm:py-4 md:px-10 md:py-6" asChild>
