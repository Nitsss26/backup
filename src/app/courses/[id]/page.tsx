...
export default function CourseDetailPage({ params }: CourseDetailPageProps) {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      ...
      try {
        const response = await axios.get<Course>(`http://localhost:9003/api/courses/${params.id}`);
        ...
        const allCoursesResponse = await axios.get<{courses: Course[]}>(`http://localhost:9003/api/courses?limit=4&category=${encodeURIComponent(response.data.category.toLowerCase().replace(/\s+/g, '-'))}`);
