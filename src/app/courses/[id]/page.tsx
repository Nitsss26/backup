...
//             <div className="md:col-span-2 space-y-4">
//               <Breadcrumbs items={breadcrumbItems.slice(0, -1)} className="mb-4 [&_a]:text-blue-100 [&_a:hover]:text-white [&_span]:text-blue-100 [&_svg]:text-blue-100"/>
//               <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold font-headline text-white">{course.title}</h1>
//               <p className="text-lg text-blue-100/90">{course.shortDescription || "An engaging online learning experience designed to elevate your skills."}</p>
//               <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-blue-50">
//                 <div className="flex items-center gap-1">
...
//                   )}
//                 </CardContent>
//                 <CardContent className="p-5 border-t text-sm space-y-2">
//                   <h4 className="font-semibold mb-2 text-base text-foreground">This course includes:</h4>
//                   {course.highlights?.slice(0,3).map((highlight, i) => (
//                     <div key={i} className="flex items-center gap-2 text-muted-foreground"><CheckCircle className="h-4 w-4 text-primary"/>{highlight}</div>
//                   ))}
//                   {course.demoVideoUrl && <div className="flex items-center gap-2 text-muted-foreground"><Video className="h-4 w-4 text-primary"/>Demo Video Available</div>}
...
//               <div className="text-2xl font-bold text-primary">₹{course.price.toLocaleString('en-IN')}
//                 {course.originalPrice && <span className="ml-2 text-base text-muted-foreground line-through">₹{course.originalPrice.toLocaleString('en-IN')}</span>}
//               </div>
//               <Button size="lg" className="w-full text-base py-2" onClick={handleAddToCart} disabled={isInCart}>
//                 <ShoppingCart className="mr-2 h-5 w-5" /> {isInCart ? "Added to Cart" : "Add to Cart"}
//               </Button>
//               <Button variant="outline" size="lg" className="w-full text-base py-2">
//                 <Heart className="mr-2 h-5 w-5" /> Add to Wishlist
//               </Button>
//               {/* {course.moneyBackGuaranteeDays && course.moneyBackGuaranteeDays > 0 && (
//                 <p className="text-xs text-center text-muted-foreground flex items-center justify-center gap-1">
...
//               )}
//             </CardContent>
//             <CardContent className="p-4 border-t text-sm space-y-2">
//               <h4 className="font-semibold mb-2 text-base text-foreground">This course includes:</h4>
//               {course.highlights?.slice(0,3).map((highlight, i) => (
//                 <div key={i} className="flex items-center gap-2 text-muted-foreground"><CheckCircle className="h-4 w-4 text-primary"/>{highlight}</div>
//               ))}
...
//           </Card>
//         </div>
//         <div className="container mt-12 grid lg:grid-cols-3 gap-6">
//           <div className="lg:col-span-2">
//             {course.highlights && course.highlights.length > 0 && (
//             <Card className="mb-12 shadow-md border bg-card">
//               <CardHeader className="p-6">
//                 <CardTitle className="text-2xl font-headline text-foreground">What you'll learn</CardTitle>
//               </CardHeader>
//               <CardContent className="p-6 grid md:grid-cols-2 gap-x-6 gap-y-3">
//                 {course.highlights.map((highlight, index) => (
//                   <div key={index} className="flex items-start gap-2.5">
//                     <CheckCircle className="h-5 w-5 text-primary mt-0.5 shrink-0" />
//                     <p className="text-sm leading-relaxed text-foreground">{highlight}</p>
...
//             <Tabs defaultValue="curriculum" className="w-full mb-12">
//               <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-5 mb-6 mx-auto max-w-4xl sticky top-16 bg-card/80 backdrop-blur-sm z-30 py-2 rounded-md shadow-sm border">
//                 <TabsTrigger value="curriculum" className="text-sm sm:text-base py-2 px-4 text-foreground hover:bg-primary/10 data-[state=active]:bg-primary/20">Curriculum</TabsTrigger>
//                 <TabsTrigger value="description" className="text-sm sm:text-base py-2 px-4 text-foreground hover:bg-primary/10 data-[state=active]:bg-primary/20">Description</TabsTrigger>
//                 <TabsTrigger value="seller" id="seller-section" className="text-sm sm:text-base py-2 px-4 text-foreground hover:bg-primary/10 data-[state=active]:bg-primary/20">Seller</TabsTrigger>
//                 <TabsTrigger value="reviews" className="text-sm sm:text-base py-2 px-4 text-foreground hover:bg-primary/10 data-[state=active]:bg-primary/20">Reviews ({reviews.length})</TabsTrigger>
//                 <TabsTrigger value="faq" className="text-sm sm:text-base py-2 px-4 text-foreground hover:bg-primary/10 data-[state=active]:bg-primary/20">FAQ</TabsTrigger>
//               </TabsList>

//               <TabsContent value="curriculum">
//                 <Card className="shadow-md border bg-card">
//                   <CardHeader className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 p-6">
//                     <div>
//                       <CardTitle className="text-2xl font-headline text-foreground">Course Content</CardTitle>
//                       <CardDescription className="mt-1">Explore the modules and lessons in this course.</CardDescription>
//                     </div>
//                     <span className="text-sm text-muted-foreground whitespace-nowrap mt-2 md:mt-0">
//                       {course.curriculum?.length || ''} modules • {course.curriculum?.reduce((acc, mod) => acc + (mod.lessons?.length || 0), 0) || ''} lessons • {course.duration}
//                     </span>
//                   </CardHeader>
//                   <CardContent className="p-6">
//                     <Accordion type="multiple" defaultValue={course.curriculum?.[0]?.id ? [String(course.curriculum[0].id)] : []} className="w-full">
...
//                     <CardTitle className="text-2xl font-headline text-foreground">Course Description</CardTitle>
//                   </CardHeader>
//                   <CardContent className="p-6 prose dark:prose-invert max-w-none text-base leading-relaxed text-foreground">
//                     <p>{course.description}</p>
//                     {course.downloadableMaterialsDescription && (
//                       <>
//                         <h3 className="font-semibold mt-6 mb-2 text-lg text-foreground">Downloadable Materials</h3>
...
//           <div className="lg:col-span-1">
//             <div className="hidden mt-12 md:mt-0"> 
//               <Card className="shadow-xl sticky top-24 border-2 border-primary/30 bg-card">
//                 <CardHeader className="p-0">
//                   <Image src={course.imageUrl || `https://placehold.co/600x338.png?text=${encodeURIComponent(course.title)}`} alt={course.title} width={600} height={338} className="rounded-t-lg object-cover w-full aspect-video" data-ai-hint={`${imageHint} course mobile cover art detail page`}/>
//                 </CardHeader>
...
export default function CourseDetailPage() {
  const params = useParams();
  const courseId = params?.id as string | undefined; // Allow undefined
  const { user } = useAuth();
  const { addToCart, cartItems } = useCart();
  const { toast } = useToast();
  ...
  const isInCart = course ? cartItems.some(item => item.course?.id === course.id) : false; // Safely handle undefined course
              ...
              <TabsContent value="curriculum">
                <Card className="shadow-md border bg-card">
                  <CardHeader className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 p-6">
                    ...
                    <span className="text-sm text-muted-foreground whitespace-nowrap mt-2 md:mt-0">
                      {course.curriculum?.length || 0} modules • {course.curriculum?.reduce((acc, mod) => acc + (mod.lessons?.length || 0), 0) || 0} lessons • {course.duration || 'N/A'}
                    </span>
                  </CardHeader>
                  <CardContent className="p-6">
                    <Accordion type="multiple" defaultValue={course.curriculum?.[0]?.id ? [String(course.curriculum[0].id)] : []} className="w-full">
              ...
              <TabsContent value="seller">
                <Card className="shadow-md border bg-card">
                  ...
                  <CardContent className="p-6 flex flex-col md:flex-row items-start gap-6">
                    ...
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-primary">{course.providerInfo?.name || course.instructor || 'Unknown Seller'}</h3>
                      {course.providerInfo?.verified && (
                        <Badge variant="success" className="mt-1 mb-2 text-xs px-2 py-0.5 border-none">
                          <ShieldCheck className="h-3.5 w-3.5 mr-1" /> Verified Seller
                        </Badge>
                      ...
                      <div className="flex flex-wrap gap-x-6 gap-y-3 mt-4 text-sm border-t pt-4">
                        <div className="flex items-center gap-1.5 text-muted-foreground"><Star className="h-4 w-4 text-yellow-400 fill-yellow-400" /> {course.rating || 0} Seller Rating (Platform Avg.)</div>
                        <div className="flex items-center gap-1.5 text-muted-foreground"><Award className="h-4 w-4 text-primary" /> {course.reviewsCount || 0} Course Reviews</div>
                        <div className="flex items-center gap-1.5 text-muted-foreground"><Users className="h-4 w-4 text-primary" /> {course.studentsEnrolledCount?.toLocaleString() || 0} Total Students on Course</div>
                      </div>
                      <Button variant="outline" size="sm" className="mt-6" disabled>View Seller Store (Coming Soon)</Button>
                    </div>
                  </CardContent>
              ...
              <TabsContent value="reviews">
                <Card className="shadow-md border bg-card">
                  <CardHeader className="p-6">
                    <CardTitle className="text-2xl font-headline text-foreground">Student Reviews</CardTitle>
                    <div className="flex items-center gap-2 mt-2">
                      <StarRating rating={course.rating || 0} size={24} />
                      <span className="text-2xl font-bold text-foreground">{(course.rating || 0).toFixed(1)}</span>
                      <span className="text-muted-foreground">({course.reviewsCount || 0} ratings)</span>
                    </div>
                  </CardHeader>
                  <CardContent className="p-6">
                    {reviews.length > 0 ? (
